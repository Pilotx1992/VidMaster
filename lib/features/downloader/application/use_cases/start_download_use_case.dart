...
class StartDownloadUseCase {
  ...
  Future<DownloadTaskEntity> call({
    ...
    final sanitizedTitle =
        result.title.replaceAll(RegExp(r'[<>:"/\\|?*]'), '_');
    ...
    final task = DownloadTaskEntity(
      ...
      engine: format.requiresMerge
          ? DownloadEngineType.ffmpeg
          : DownloadEngineType.native,
      totalBytes: format.fileSizeBytes,
    );

    // 3. Persist initial record to DB
    DownloaderLog.queue('Persisting download task $taskId for $fileName');
    await _repo.save(task);

    // 4. Start native download
    if (format.requiresMerge) {
      DownloaderLog.queue('Starting DASH child downloads for $taskId');
      final (videoNativeId, audioNativeId) =
          await _startDashDownload(taskId, result, format);
      ...
      DownloaderLog.queue('Starting native download for $fileName');
  ...
  Future<(String?, String?)> _startDashDownload(
    ...
    final videoTemp = await _storage
        .resolveTempPath('${logicalTaskId}_video.${format.extension}');
